...
| $$  \ $$| $$ /$$__  $$| $$      | $$_  $$ | $$\  $$ | $$| $$  | $$| $$  | $$
| $$$$$$$/| $$|  $$$$$$$|  $$$$$$$| $$ \  $$| $$ \  $$| $$| $$  | $$|  $$$$$$$
|_______/ |__/ \_______/ \_______/|__/  \__/|__/  \__/|__/|__/  |__/ \____  $$
                                                                     /$$  \ $$
                                                                    |  $$$$$$/
                                                                     \______/                                                              
...
if getgenv then
	...
	local Success, Error = pcall(function()
		if ((typeof(getgenv()) == "table") and getgenv().getgenv) then
			getgenv().BlackKing_GENV_TEST = "BlackKing_GENV_TEST_VALUE";
			if (getgenv().BlackKing_GENV_TEST == "BlackKing_GENV_TEST_VALUE") then
				Test1 = true;
			end

			if (BlackKing_GENV_TEST and BlackKing_GENV_TEST  == "BlackKing_GENV_TEST_VALUE") then
				Test2 = true;
			end
			getgenv().BlackKing_GENV_TEST = nil;
		end
	end);
...
function CheckHookMetaMethod()
	...
	if (hookmetamethod and ExecutorSupport['newcclosure']) then
		local object = setmetatable({}, {__index=newcclosure(function()
			return false;
		end),__metatable="BlackKing_METATABLE_TEST"});
		local ref;
		local Success, Error = pcall(function()
...
function CheckRequire()
	...
	local Success, Error = pcall(function()
		...
		LoadedModule.GetControls = function()
			return "BlackKing_REQUIRE_TEST";
		end;
		if (LoadedModule:GetControls() == "BlackKing_REQUIRE_TEST") then
			Test = true;
...
function CheckGetRawMetaTable()
	if getrawmetatable then
		local metatable = {__metatable="BlackKing_METATABLE_TEST"};
...
function CheckSetRawMetaTable()
	if setrawmetatable and ExecutorSupport["getrawmetatable"] then
		local metatable1 = {__metatable="BlackKing_METATABLE_TEST_1"};
		local metatable2 = {__metatable="BlackKing_METATABLE_TEST_2"};
...
if request then
	...
	local Success, Error = pcall(function()
		local response = request({
			Url = "https://raw.githubusercontent.com/notgiovani/ExecutorSupport/refs/heads/main/main.lua",
			Method = "GET",
		})
...
if clonefunction then
	...
	local TestFunction = function()
		return "BlackKing_FUNCTION_TEST";
	end;
	local Success, Error = pcall(function()
		ClonedFunction = clonefunction(TestFunction);
	end);
	if (Success and (ClonedFunction() == "BlackKing_FUNCTION_TEST") and (ClonedFunction ~= TestFunction)) then
...
if loadstring then
	...
	local Success, Error = pcall(function()
		loadstring([[workspace.Name = "BlackKing_LOADSTRING_TEST"]])();
	end);
	if Success then
...
if gethiddenproperty then
	...
	local Success2, Error2 = pcall(function()
		Property2 = gethiddenproperty(NewPart, "BlackKing_FAKE_PROPERTY");
	end);
...
if writefile then
	local Success, Error = pcall(function()
		writefile("BlackKing_TEST_FILE", "BlackKing_FILE_CONTENTS");
	end);
	if Success then
	...
	if isfile then
		local Success, Error = pcall(function()
			isfile("BlackKing_TEST_FILE");
		end);
		if (Success and isfile("BlackKing_TEST_FILE")) then
...
if readfile then
	local Success, Error = pcall(function()
		readfile("BlackKing_TEST_FILE");
	end);
	if (Success and (readfile("BlackKing_TEST_FILE") == "BlackKing_FILE_CONTENTS")) then
		ExecutorSupport['readfile'] = true;
	end
end
if loadfile and ExecutorSupport["writefile"] then
	local Success, Error = pcall(function()
		writefile("BlackKing_TEST_FILE_3", "game:GetService('CoreGui').Name = 'BlackKing_LOADFILE_TEST'")
		local Chunk = loadfile("BlackKing_TEST_FILE_3")
		Chunk()
	end);
	if (Success and (game:GetService("CoreGui").Name == "BlackKing_LOADFILE_TEST")) then
...
if makefolder then
	local Success1, Error1 = pcall(function()
		makefolder("BlackKing_TEST_FOLDER");
	end);
	if (Success1 and ExecutorSupport['writefile']) then
		writefile("BlackKing_TEST_FOLDER/BlackKing_TEST_FILE", "BlackKing_FILE_CONTENTS");
	end
	if Success1 then
		local Success2, Error2 = pcall(function()
			readfile("BlackKing_TEST_FOLDER/BlackKing_TEST_FILE");
		end);
...
if isfolder then
	...
	local Success1, Error1 = pcall(function()
		FolderValue = isfolder("BlackKing_TEST_FOLDER");
	end);
	if Success1 then
		if isfolder("BlackKing_TEST_FILE") == false then
...
if delfolder then
	local Success1, Error1 = pcall(function()
		delfolder("BlackKing_TEST_FOLDER");
	end);
	if Success1 then
		if Success1 then
...
if (appendfile and ExecutorSupport['readfile']) then
	local Success, Error = pcall(function()
		appendfile("BlackKing_TEST_FILE", "_2");
	end);
	if (Success and (readfile("BlackKing_TEST_FILE") == "BlackKing_FILE_CONTENTS_2")) then
		ExecutorSupport['appendfile'] = true;
	end
end
if (delfile and ExecutorSupport['readfile']) then
	local Success1, Error1 = pcall(function()
		delfile("BlackKing_TEST_FILE");
	end);
	if Success1 then
		local Success2, Error2 = pcall(function()
			readfile("BlackKing_TEST_FILE");
...
if workspace.Name == "BlackKing_LOADSTRING_TEST" and LoadstringPassed == true then
